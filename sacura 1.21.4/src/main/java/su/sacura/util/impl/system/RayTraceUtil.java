...
public class RayTraceUtil implements MinecraftWrapper {
    ...
    public static float getHitProgress(Entity entity) {
        ...
        if (hitTime == null) return 0.0F;
        long elapsed = System.currentTimeMillis() - hitTime;
        if (elapsed > 200L) { lastHitTimes.remove(entity.getUuid()); return 0.0F; }
    ...
    public static Entity getMouseOver(Entity target, float yaw, float pitch, double distance) {
        ...
        if (playerBox.intersects(targetBox)) {
            ...
            if (hitResult != null && hitResult.getEntity() == target) return target;
        }
        ...
        if (entityHitResult != null && startVec.distanceTo(entityHitResult.getPos()) <= distance) return entityHitResult.getEntity();
        return null;
    }
    ...
    private static EntityHitResult rayCastEntities(Entity source, Vec3d start, Vec3d end, Box boundingBox, Predicate<Entity> predicate, double maxDistance) {
        World world = source.getWorld();
        double closestDistance = maxDistance;
        Entity closestEntity = null;
        Vec3d closestHitPos = null;
        // Исправлено: getOtherEntities(Entity.class, ...) -> getOtherEntities(source, ...)
        for (Entity entity : world.getOtherEntities(source, boundingBox, predicate)) {
        ...
        if (closestEntity != null) return new EntityHitResult(closestEntity, closestHitPos);
        return null;
    }
    ...
    public static BlockHitResult rayCast(double range, float yaw, float pitch, boolean includeFluids) {
        ...
        // Исправлено: field_1347 -> ANY, field_1348 -> NONE
