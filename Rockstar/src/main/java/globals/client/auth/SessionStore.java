...
final class SessionStore {
    private static final File FILE = new File(ConfigManager.internalField0148, "session.json");
    ...
    static void save(Session session) {
        ...
        try {
            ...
            ConfigManager.internalMethod01467(FILE, (JsonElement)jsonObject2);
        }
        catch (Exception exception) {
            // empty catch block
        }
    ...
    private static SecretKeySpec key() throws Exception {
        String string = String.join((CharSequence)"|", String.valueOf(System.getProperty("user.name")), String.valueOf(System.getenv("COMPUTERNAME")), String.valueOf(System.getenv("USERDOMAIN")), ConfigManager.internalField0148.getAbsolutePath());
