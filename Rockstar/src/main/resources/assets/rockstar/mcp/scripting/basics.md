# Скрипты для чит-клиента NoName (Minecraft)
...
def on_render(event):
    ...
    ctx.drawText(font("medium", 8), "NoName", 98, 98, white)
...
| `profile` / `account` / `user` | аккаунт игрока: `username`, `uid`, `role`, `staff`, `subscription`, `days_left` (свойства, без скобок) |
