...
def draw(d):                                       # координаты ОТНОСИТЕЛЬНЫЕ
    ...
    d.text("NoName", 6, 5, size=7, color=Color(207, 215, 251))
```
...
- `d.image(texture, x, y, w, h, radius=0, color)` (алиас `d.texture`) — ассет из `assets.image(...)`, обложка трека
...
def draw(e):
    ...
    ctx.drawText(font("medium", 8), "NoName", 16, 16, Color(255, 255, 255))
```
