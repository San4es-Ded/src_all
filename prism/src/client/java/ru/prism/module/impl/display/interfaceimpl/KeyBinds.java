...
public class KeyBinds implements element {
    ...
    private ru.prism.utils.animation.satoshi.Animation animation1 = new EaseInOutQuad(300, 1);
    private ru.prism.utils.animation.satoshi.Animation animation2 = new EaseInOutQuad(300, 1);
    ...
    public void onRender(DragSetting dragSetting, InterFace interFace) {
        ...
        List<ru.prism.module.api.Module> modules = Client.get().moduleManager().values().stream()
                .filter(m -> m.getKey() > 0
                        && !m.getName().equalsIgnoreCase("ClickGui")
                        && (m.isEnabled() || m.getAnimation().getOutput() > 0))
                .sorted(Comparator.comparingInt((ru.prism.module.api.Module m) ->
