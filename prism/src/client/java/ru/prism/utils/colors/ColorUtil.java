...
public class ColorUtil {
    ...
    public int client() {
        ru.prism.manager.Theme theme = getTheme();
        ru.prism.manager.Theme preTheme = getPreTheme();
        int target = theme.getClient();
        int pre = preTheme.getClient();

        ru.prism.Client client = ru.prism.Client.get();
        if (client != null && client.guiManager() != null) {
            if (theme == ru.prism.manager.Theme.CUSTOM) target = client.guiManager().getCustomColor();
            if (preTheme == ru.prism.manager.Theme.CUSTOM) pre = client.guiManager().getCustomColor();
        }

        return interpolate(target, pre, 1 - Menu.animation14.getOutput());
    }
    ...
    private static ru.prism.manager.Theme getTheme() {
        return ru.prism.screen.Menu.selectedTheme != null ? ru.prism.screen.Menu.selectedTheme : ru.prism.manager.Theme.NIGHT;
    }

    private static ru.prism.manager.Theme getPreTheme() {
        return Menu.preSelectedTheme != null ? Menu.preSelectedTheme : ru.prism.manager.Theme.NIGHT;
    }
