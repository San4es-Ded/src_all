...
import ru.prism.utils.taskript.Script;
...
//
    ...
    public void onEvent(MouseScrollEvent event) {
        ...
        if (interFace == null || !interFace.isEnabled()) {
            return;
        }
        if (!interFace.element.getValue("Music Player")) {
            return;
        }

        final DragSetting dragSetting = interFace.music;
        final Vector2f mouse = getMouse((float) event.getMouseX(), (float) event.getMouseY());
        if (!isHover(mouse.x, mouse.y, dragSetting.position.x, dragSetting.position.y,
                dragSetting.size.x, dragSetting.size.y)) {
            return;
        }

        final SliderSetting scale = interFace.musicScale;
        final float direction = event.getVerticalAmount() > 0 ? 1.0F : -1.0F;
        final float oldScale = scale.getValue();
        final float newScale = MathUtil.clamp(oldScale + direction * scale.increment, scale.min, scale.max);

        if (Math.abs(newScale - oldScale) >= 0.0001F) {
            scale.set(newScale);
            GuiSounds.sliderTick((newScale - scale.min) / (scale.max - scale.min));
        }
