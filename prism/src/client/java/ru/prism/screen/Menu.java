...
import ru.prism.screen.editor.OverlayEditor;
import ru.prism.screen.editor.OverlayEditors;
...
public class Menu extends Screen implements IMinecraft {
    ...
    public static ru.prism.manager.Theme selectedTheme;
    public static ru.prism.manager.Theme preSelectedTheme;
    public static ru.prism.manager.Theme[] themes;
    public static ru.prism.utils.animation.satoshi.Animation animation14 = new EaseInOutQuad(300, 1);
    public static ru.prism.utils.animation.satoshi.Animation animCategoryReset = new EaseInOutQuad(300, 1);
    public static ru.prism.utils.animation.satoshi.Animation animUserInfo = new EaseInOutQuad(300, 1);
    public static ru.prism.utils.animation.satoshi.Animation animThemePanel = new EaseInOutQuad(300, 1, Direction.BACKWARDS);

    /** Палитра тем раскрывается только по клику на иконку-кнопку. */
    private static boolean showThemes = false;

    /** Свой цвет: кольцо-превью и раскрытый HSB-пикер. */
    private static int customPickerColor = ru.prism.manager.Theme.customColor;
    private static boolean customPickerOpen = false;
    private static final ru.prism.utils.animation.satoshi.Animation animCustomPicker = new EaseInOutQuad(220, 1, Direction.BACKWARDS);
    private boolean draggingCustomColor = false;
    private int draggingCustomBar = 0;

    public static ru.prism.utils.animation.satoshi.Animation animation1 = new EaseInOutQuad(300, 1);
    public static ru.prism.utils.animation.satoshi.Animation animation2 = new EaseInOutQuad(300, 1);
    public static ru.prism.utils.animation.satoshi.Animation animation3 = new EaseInOutQuad(300, 1);
    public static ru.prism.utils.animation.satoshi.Animation animation4 = new EaseInOutQuad(300, 1);
    ...
    private final ru.prism.utils.animation.satoshi.Animation animSearchFocus = new EaseInOutQuad(300, 1, Direction.BACKWARDS);
    private final ru.prism.utils.animation.satoshi.Animation animSearchText = new EaseInOutQuad(300, 1, Direction.BACKWARDS);
    private final ru.prism.utils.animation.satoshi.Animation animSearchEmpty = new EaseInOutQuad(300, 1, Direction.BACKWARDS);

    private static final int SEARCH_LIMIT = 24;

    private final HashMap<String, ru.prism.utils.animation.satoshi.Animation> chipAnims = new HashMap<>();

    private ru.prism.utils.animation.satoshi.Animation chipAnim(String key) {
    ...
    private final java.util.IdentityHashMap<Setting<?>, ru.prism.utils.animation.satoshi.Animation> visAnims = new java.util.IdentityHashMap<>();

    private float visAnim(Module f, Setting<?> setting) {
        ru.prism.utils.animation.satoshi.Animation a = visAnims.computeIfAbsent(setting, k -> new EaseInOutQuad(300, 1));
    ...
    protected void init() {
        ...
        customPickerColor = Client.get().guiManager().getCustomColor();
        showThemes = false;
        customPickerOpen = false;
        themes = ru.prism.manager.Theme.values();
    }
    ...
    public void renderOverlay(DrawContext context, RenderTickCounter tickCounter) {
        ...
        animThemePanel.setDirection(showThemes ? Direction.FORWARDS : Direction.BACKWARDS);
        float panelAnim = animThemePanel.getOutput();
        animCustomPicker.setDirection(customPickerOpen ? Direction.FORWARDS : Direction.BACKWARDS);
        float customOpen = animCustomPicker.getOutput();

        if (panelAnim > 0.01F) {
            RenderUtil.Render2D.glow(xt, yt, wt, ht - 0.5F * S, ColorUtil.getColor(0, 0.15F * globalAnim * panelAnim), 7.5F * S, 15, 1);
            RenderUtil.Blur.blur(xt, yt, wt, ht, globalAnim * panelAnim, 7.5F * S, ColorUtil.multAlpha(ColorUtil.multDark(ColorUtil.background(), 0.6F), globalAnim * panelAnim));
            RenderUtil.Images.texture(Identifier.of("client","textures/frame/rectthemegui.png"), xt, yt, wt, ht, ColorUtil.multAlpha(ColorUtil.client(), globalAnim * panelAnim));
            ...
            for (Theme theme : themes) {
                ...
                int themeColor = theme == Theme.CUSTOM ? customPickerColor : theme.getClient();

                RenderUtil.Render2D.rect(xtd + 0.75F * S * anPC, ytd + 0.75F * S * anPC, 8 * S - 1.5F * S * anPC, 8 * S - 1.5F * S * anPC, ColorUtil.replAlpha(themeColor, (0.5F + 0.5F * anPC) * globalAnim * panelAnim), 8 * S);
                RenderUtil.Render2D.outline(xtd - 0.5F * S * anPC, ytd - 0.5F * S * anPC, 8 * S + 1 * S * anPC, 8 * S + 1 * S * anPC, 0.25F * S, ColorUtil.replAlpha(themeColor, (1.0F * anPC) * globalAnim * panelAnim), 8 * S);
                RenderUtil.Render2D.glow(xtd + 0.75F * S * anPC, ytd + 0.75F * S * anPC, 8 * S - 1.5F * S * anPC, 8 * S - 1.5F * S * anPC, ColorUtil.replAlpha(themeColor, (0.1F * anPC) * globalAnim * panelAnim), 8 * S, 7, 1);
            ...
            if (customOpen > 0.01F) {
                ...
                RenderUtil.Render2D.glow(px, py, pw, ph - 0.5F * S, ColorUtil.getColor(0, 0.15F * globalAnim * panelAnim * customOpen), 7.5F * S, 15, 1);
                RenderUtil.Blur.blur(px, py, pw, ph, globalAnim * panelAnim * customOpen, 7.5F * S, ColorUtil.multAlpha(ColorUtil.multDark(ColorUtil.background(), 0.6F), globalAnim * panelAnim * customOpen));
                RenderUtil.Images.texture(Identifier.of("client","textures/frame/rectthemegui.png"), px, py, pw, ph, ColorUtil.multAlpha(ColorUtil.client(), globalAnim * panelAnim * customOpen));

                float[] hsb = java.awt.Color.RGBtoHSB((customPickerColor >> 16) & 0xFF, (customPickerColor >> 8) & 0xFF, customPickerColor & 0xFF, null);

                for (int bar = 0; bar < 3; bar++) {
                    int segs = 16;
                    float seg = bwd / segs;
                    float bby = py + 4 * S + bar * 4.5F * S;

                    for (int i = 0; i < segs; i++) {
                        float t0 = (float) i / segs;
                        float t1 = (float) (i + 1) / segs;
                        int c0 = switch (bar) { case 0 -> java.awt.Color.HSBtoRGB(t0, 1F, 1F); case 1 -> java.awt.Color.HSBtoRGB(hsb[0], t0, hsb[2]); default -> java.awt.Color.HSBtoRGB(hsb[0], hsb[1], t0); };
                        int c1 = switch (bar) { case 0 -> java.awt.Color.HSBtoRGB(t1, 1F, 1F); case 1 -> java.awt.Color.HSBtoRGB(hsb[0], t1, hsb[2]); default -> java.awt.Color.HSBtoRGB(hsb[0], hsb[1], t1); };

                        RenderUtil.Render2D.gradientRect(bx + i * seg, bby, seg + (i == segs - 1 ? 0 : 0.5F * S), 2.5F * S, new int[]{ColorUtil.replAlpha(c0, globalAnim * panelAnim * customOpen), ColorUtil.replAlpha(c1, globalAnim * panelAnim * customOpen), ColorUtil.replAlpha(c1, globalAnim * panelAnim * customOpen), ColorUtil.replAlpha(c0, globalAnim * panelAnim * customOpen)}, i == 0 ? 1.25F * S : 0, i == segs - 1 ? 1.25F * S : 0, i == segs - 1 ? 1.25F * S : 0, i == 0 ? 1.25F * S : 0);
                    }

                    float kx = bx + bwd * hsb[bar];
                    int kc = switch (bar) { case 0 -> java.awt.Color.HSBtoRGB(hsb[0], 1F, 1F); default -> customPickerColor; };
                    RenderUtil.Render2D.rect(kx - 1.5F * S, bby - 1 * S, 3 * S, 4.5F * S, ColorUtil.getColor(255, globalAnim * panelAnim * customOpen), 1.5F * S);
                    RenderUtil.Render2D.rect(kx - 1 * S, bby, 2 * S, 2.5F * S, ColorUtil.replAlpha(kc, globalAnim * panelAnim * customOpen), 1.5F * S);
                }
            }
        }
        ...
        String sgff = "PrismVisuals" + ColorFormatting.getColor(ColorUtil.replAlpha(ColorUtil.client(), globalAnim * animL)) + " 5.0";
        ...
        for (Category category : Category.values()) {
            ...
            ru.prism.utils.animation.satoshi.Animation animUF = category.alphaS2;
            ...
            Fonts.prism_2.drawCentered(name, xPanelMini + (30 * S) / 2, cy + 4.5F * S, 8 * S, ColorUtil.multAlpha(ColorUtil.overCol(ColorUtil.getColor(255, 0.15F * globalAnim + 0.5F * animL2), ColorUtil.client(), anim), globalAnim));
        ...
        Fonts.prism_2.draw(catName, catPanX + 8 * S, y + 12.7F * S + 5 * S - 5 * S * animCategoryReset.getOutput(), 7 * S, ColorUtil.multAlpha(ColorUtil.client(), globalAnim * animCategoryReset.getOutput()));
        ...
        for (Module f : Client.get().moduleManager().values()) {
            ...
            if (canim1 > 0) {
                ...
                ru.prism.utils.animation.satoshi.Animation bindAct = chipAnim(f.getName() + ":modbind");
                ...
                ru.prism.utils.animation.satoshi.Animation bindHov = chipAnim(f.getName() + ":modbindhov");
                ...
                for (Setting setting : f.getSettings()) {
                        ...
                        if (vis > 0.01F) {
                            ...
                            if (onScreen) {
                                ...
                                for (String val : s.values) {
                                    ...
                                    ru.prism.utils.animation.satoshi.Animation chip = chipAnim(f.getName() + ":" + s.getName() + ":" + val);
                        ...
                        if (vis > 0.01F) {
                            ...
                            if (onScreen) {
                                ...
                                for (BooleanSetting b : s.getValues()) {
                                    ...
                                    float cx = xST + 6 * S + px;
                                    float cyy = yST + 15 * S + py;
                                    ru.prism.utils.animation.satoshi.Animation chip = chipAnim(f.getName() + ":" + s.getName() + ":" + b.getName());
                                    chip.setDirection(b.getValue() ? Direction.FORWARDS : Direction.BACKWARDS);
                                    float sel = chip.getOutput();

                                    RenderUtil.Render2D.rect(cx, cyy, tw, 10 * S, ColorUtil.overCol(ColorUtil.getColor(0, 0.25F * globalAnim * sa), ColorUtil.replAlpha(ColorUtil.client(), globalAnim * sa * (0.5F + 0.5F * hover)), sel), 3 * S);
                                    regular.drawCentered(b.getName(), cx + tw / 2F, cyy + 1.5F * S, 6 * S, ColorUtil.getColor(255, globalAnim * sa * (0.35F + 0.65F * sel) * (0.7F + 0.3F * hover)));
                                    px += tw + 3 * S;
                                }
                            }
                    ...
                    if (setting instanceof ButtonSetting s) {
                        ...
                        if (vis > 0.01F) {
                            ...
                            boolean onScreen = yST + 16 * S >= setTop && yST <= setBottom;
                            boolean hovS = onScreen && MathUtil.isHovered((float) lastMouseX, (float) lastMouseY, xST, yST, wST, 16 * S);
                            ru.prism.utils.animation.satoshi.Animation btn = chipAnim(f.getName() + ":" + s.getName() + ":btn");
                    ...
                    if (setting instanceof BindSetting s) {
                        ...
                        if (vis > 0.01F) {
                            ...
                            if (onScreen) {
                                ...
                                ru.prism.utils.animation.satoshi.Animation bindAct = chipAnim(f.getName() + ":" + s.getName() + ":act");
                    ...
                    if (setting instanceof StringSetting s) {
                        ...
                        if (vis > 0.01F) {
                            ...
                            if (onScreen) {
                                ...
                                ru.prism.utils.animation.satoshi.Animation editAct = chipAnim(f.getName() + ":" + s.getName() + ":act");
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (MathUtil.isHovered(mouseX, mouseY, xPanelMini + (30 * S) / 2 - (12 * S) / 2, yPanelMini + 7 * S, 12 * S, 12 * S) && click.button() == 0) {
            showThemes = !showThemes;
            GuiSounds.expand(showThemes);
            return true;
        }
        ...
        if (showThemes) {
            ...
            for (Theme theme : themes) {
                if (MathUtil.isHovered(mouseX, mouseY, xtd, ytd, 8 * S, 8 * S) && click.button() == 0) {
                    if (theme == Theme.CUSTOM) {
                        ...
                        GuiSounds.picker(customPickerOpen);
                    } else {
                        customPickerOpen = false;
                    }
            ...
            if (customPickerOpen) {
                ...
                for (int bar = 0; bar < 3; bar++) {
                    if (MathUtil.isHovered(mouseX, mouseY, bx, py + 4 * S + bar * 4.5F * S - 1 * S, bwd, 4.5F * S)) {
                        draggingCustomColor = true; draggingCustomBar = bar; GuiSounds.sliderGrab();
                        return true;
                    }
                }
            }
        ...
        for (Module f : Client.get().moduleManager().values()) {
            ...
            if (canim1 > 0) {
                ...
                if (insidePanel && canim1 > 0.5F && MathUtil.isHovered(mouseX, mouseY, xModule, yModule, 130 * S, moduleH)) {
                    ...
                    if (click.button() == 1) { select = (f == select) ? null : f; settingScrollTarget = 0; settingScrollAnim = 0; GuiSounds.expand(select == f); if (select != null && select == ru.prism.module.impl.render.Cosmetics.getInstance()) { net.minecraft.client.MinecraftClient minecraft = net.minecraft.client.MinecraftClient.getInstance(); minecraft.setScreen(new CosmeticsPreviewScreen(minecraft.currentScreen)); } }
                }
                yModule += (moduleH + 5 * S) * canim1;
            }
        }
    ...
    public boolean mouseDragged(Click click, double deltaX, double deltaY) {
        ...
        if (draggingCustomColor) {
            ...
            float[] hsb = java.awt.Color.RGBtoHSB((customPickerColor >> 16) & 0xFF, (customPickerColor >> 8) & 0xFF, customPickerColor & 0xFF, null);
            float t = MathUtil.clamp((mouseX - bx) / bwd, 0, 1);
            float[] nh = {hsb[0], hsb[1], hsb[2]};
            nh[draggingCustomBar] = t;
            int rgb = java.awt.Color.HSBtoRGB(nh[0], nh[1], nh[2]);
            customPickerColor = 0xFF000000 | (rgb & 0x00FFFFFF);
            Client.get().guiManager().setCustomColor(customPickerColor);
            if (selectedTheme == Theme.CUSTOM) animation14.reset();
    ...
    public boolean mouseReleased(Click click) {
        ...
        if (draggingSlider != null || draggingColor != null || draggingCustomColor) GuiSounds.sliderRelease();
        draggingSlider = null; draggingColor = null; draggingCustomColor = false;
