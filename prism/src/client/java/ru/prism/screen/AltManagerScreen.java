...
import ru.prism.alt.Account;
import ru.prism.alt.AltManager;
...
public class AltManagerScreen extends Screen implements IMinecraft {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (accs.isEmpty()) {
            font.drawCentered(ru.prism.lang.Lang.pick("Пока нет аккаунтов", "Пока нет аккаунтов"), listX + listW / 2F, listY + listH / 2F - 4, 12, ColorUtil.getColor(255,a * 0.15F));
        }
        Scissor.disable();
     ...
     //           ColorUtil.replAlpha(ColorUtil.getColor(255, 255, 255, 1F), a * (0.015F + 0.12F * gp)), 4);
        ...
        drawButton(addX, addY, addW, addH, ru.prism.lang.Lang.pick("Добавить аккаунт", "Добавить аккаунт"), a, MathUtil.clamp(addAnim.get(), 0F, 1F), false);
        drawButton(genX, genY, genW, genH, ru.prism.lang.Lang.pick("Случайный аккаунт", "Случайный аккаунт"), a, MathUtil.clamp(genAnim.get(), 0F, 1F), false);
    ...
    private void renderSidePanel(float a, Font font) {
        ...
        float pad = 12;
        font.draw(ru.prism.lang.Lang.pick("Настройки рандома", "Random settings"), sideX + pad, sideY + 9, 8, ColorUtil.getColor(185, sa));
        font.draw(ru.prism.lang.Lang.pick("Начальный ник", "Initial nickname"), sideX + pad, sideY + 26, 6.5F, ColorUtil.getColor(185, sa));
        ...
        String shown = randomBase.isEmpty() && !baseFocused ? ru.prism.lang.Lang.pick("напр. night", "e.g. night") : randomBase + (blink ? "_" : "");
        font.draw(shown, baseX + 8, baseY + 6F, 6.5F,
                randomBase.isEmpty() && !baseFocused ? ColorUtil.getColor(140, sa) : ColorUtil.getColor(185, sa));

        font.draw(ru.prism.lang.Lang.pick("+ 4-12 случайных символов", "+ 4-12 random chars"), sideX + pad, sideY + sideH - 16, 7, ColorUtil.getColor(185, sa * 0.5F));
    }
