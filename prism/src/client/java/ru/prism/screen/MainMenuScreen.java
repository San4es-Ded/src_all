...
public class MainMenuScreen extends Screen implements IMinecraft {
    ...
    private final Animation exitSlideAnim = new Animation();
    private double exitDragStartX = 0;
    private float exitDragStartProgress = 0F;
    ...
    protected void init() {
        ...
        ru.prism.alt.AltManager.get().bootstrap();
    ...
    public void renderOverlay(DrawContext context) {
        ...
        // Заголовок привязываем к верхней границе блока кнопок, а не к высоте экрана:
        // иначе в маленьком окне логотип наезжает на ряд кнопок.
        float buttonsTop = screenHeight / 2F - (25F + 4F + 25F + 4F + 25F) / 2F + (1F - alphaVal) * 34F;
        float logoY = Math.max(4F, buttonsTop - 40F);

        Fonts.prism_2.drawCentered("G", screenWidth / 2,
                logoY, 16, ColorUtil.replAlpha(ColorUtil.client(), alphaVal));

        Fonts.sf_regular.drawCentered("PrismVisuals", screenWidth / 2,
                logoY + 20F, 12, ColorUtil.getColor(255, alphaVal));
    ...
    private void drawMenuButtons(int screenWidth, int screenHeight, float alphaVal) {
        ...
        exitButton.drawExit(exitX, currentY, exitWidth, exitHeight, alphaVal, exitSlideAnim.get(), exitSlideDragging);
    }
    ...
    private void updateExitSlideProgress() {
        ...
        exitSlideProgress = Math.max(0F, Math.min(1F,
                exitDragStartProgress + (float) (lastMouseX - exitDragStartX) / trackW));
    }

    /** Плавно подтягиваем отображаемую позицию ползунка к целевой. */
    private void updateExitSlideAnimation() {
        exitSlideAnim.update();
        exitSlideAnim.run(exitSlideProgress, exitSlideDragging ? 0.12F : 0.25F, Easings.SINE_OUT);
    }
    ...
    private void drawLanguageButton(int screenWidth, float alphaVal) {
        ...
        boolean hov = ru.prism.utils.math.MathUtil.isHovered((float) lastMouseX, (float) lastMouseY, bx, by, w, h);
        langHover.update();
        ...
        Fonts.sf_regular.drawCentered(ru.prism.lang.Lang.tag(), bx + w / 2F, by + h / 2F - 3.8F, 6, textColor);
    }
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (button == 0) {
            if (ru.prism.utils.math.MathUtil.isHovered(mouseX, mouseY, langBtnX, langBtnY, langBtnW, langBtnH)) {
                ru.prism.lang.Lang.toggle();
                return true;
            }
            for (MenuButton btn : buttons) {
                if (btn.isHovered()) {
                    if (btn == exitButton) {
                        if (btn.isKnobHovered(mouseX, mouseY)) {
                            exitSlideDragging = true;
                            exitDragStartX = mouseX;
                            exitDragStartProgress = exitSlideProgress;
    ...
    private static class MenuButton {
        ...
        private float knobX, knobY, knobSize;
        ...
        private String description() { return ru.prism.lang.Lang.pick(descRu, descEn); }
        ...
        public boolean isKnobHovered(double mouseX, double mouseY) {
            return MathUtil.isHovered((float) mouseX, (float) mouseY, knobX, knobY, knobSize, knobSize);
        }

        public float getKnobCenterX() {
            return knobX + knobSize / 2F;
        }
        ...
        public void drawExit(float x, float y, float width, float height, float globalAlpha, float slideProgress, boolean dragging) {
            ...
            Fonts.sf_regular.drawCentered(ru.prism.lang.Lang.pick("Сдвиньте, чтобы выйти", "Сдвиньте, чтобы выйти"), x + width / 2F + 8F,
                    y + height / 2F - 4.4F, 7,
