...
// Функции эффектов перенесены как есть, поэтому имена входов оставлены прежними.
#define baseColor TintColor
...
// ==================== Маска руки ====================
// Рука — это пиксели, глубина которых изменилась после снимка, сделанного до её отрисовки.
...
void main() {
    ...
    if (Blend > 0.5) {
        OutColor = vec4(effectColor, TintColor.a * mask);
    } else {
        vec3 base = texture(Sampler0, uv).rgb;
        OutColor = vec4(base + effectColor * TintColor.a, mask);
    }
}
