...
public class ConfigCommand extends CommandAbstract {
   ...
   public void execute(LiteralArgumentBuilder<CommandSource> builder) {
      builder.then(literal("list").executes(context -> {
         java.util.List<String> configs = Wyvern.getInstance().getConfigManager().configNames();
         if (configs.isEmpty()) {
            MessageUtil.displayInfo("Сохранённых конфигов нет");
         } else {
            MessageUtil.displayInfo("Конфиги: " + String.join(", ", configs));
         }
         return 1;
      }));
