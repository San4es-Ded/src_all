...
public class RequestHandler<T> {
   ...
   private final PriorityBlockingQueue<Request<T>> activeRequests = new PriorityBlockingQueue(11, Comparator.comparingInt((r) -> {
   ...
   public T getActiveRequestValue() {
      Request<T> top = (Request)this.activeRequests.peek();
      if (top == null) {
         return null;
      } else {
         if (MinecraftClient.getInstance().isOnThread()) {
            while(top != null && (top.expiresIn <= this.currentTick || !top.provider.isEnabled())) {
               ...
               top = (Request)this.activeRequests.peek();
            }
         }
