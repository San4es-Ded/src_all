...
public final class ClickGuiLayout {
    ...
    // Chip heights are pure functions of the (fixed) value names and layout
    // constants; they were re-measured per module per frame. Cache per setting.
    private static final java.util.Map<Object, CachedChipHeight> CHIP_HEIGHT_CACHE = new java.util.IdentityHashMap<>();

    public static float calculateModeSettingHeight(ModeSetting modeSetting) {
        int valueCount = modeSetting.getValues().size();
        CachedChipHeight cached = CHIP_HEIGHT_CACHE.get(modeSetting);
        ...
        float height = 9.0f + (rows * rowHeight) + ((rows - 1) * CHIP_GAP_Y) + 2.0f;
        CHIP_HEIGHT_CACHE.put(modeSetting, new CachedChipHeight(valueCount, height));
        return height;
    }

    public static float calculateMultiBooleanHeight(MultiBooleanSetting multiBooleanSetting) {
        int valueCount = multiBooleanSetting.getBooleanSettings().size();
        CachedChipHeight cached = CHIP_HEIGHT_CACHE.get(multiBooleanSetting);
        if (cached != null && cached.valueCount() == valueCount) {
            return cached.height();
        }
        float x = SETTING_LEFT;
        ...
        float height = 9.0f + (rows * rowHeight) + ((rows - 1) * CHIP_GAP_Y) + 2.0f;
        CHIP_HEIGHT_CACHE.put(multiBooleanSetting, new CachedChipHeight(valueCount, height));
        return height;
    }
    ...
    public static float calculateSettingsHeight(Module module) {
        ...
        // Plain loop instead of a stream+list allocation per module per frame.
        List<Setting> visibleSettings = new java.util.ArrayList<>(settings.size());
        for (Setting setting : settings) {
        ...
        for (int i = 0; i < visibleSettings.size(); i++) {
            ...
            if (setting instanceof BooleanSetting || setting instanceof BindSetting) {
                currentHeight = 12f;
            } else if (setting instanceof SliderSetting || setting instanceof StringSetting) {
                currentHeight = 22f;
