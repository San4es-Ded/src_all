...
public class ClickGuiSettingRenderer {
    private static final Map<String, TruncatedName> NAME_TRUNCATION_CACHE = new java.util.HashMap<>();

    private record TruncatedName(float maxWidth, String text, float width) { }
    ...
    public void render(DrawContext context, Module module, float panelX, float moduleY, float openProgress, int colorTheme, double mouseX, double mouseY, ClickGuiState state, float alphaMul) {
        ...
        for (int i = 0; i < visibleSettings.size(); i++) {
            ...
            } else if (setting instanceof SliderSetting floatSetting) {
    ...
    private void renderBooleanSetting(DrawContext context, float panelX, float settingY, int alpha, int colorTheme, double mouseX, double mouseY, BooleanSetting booleanSetting, ClickGuiState state) {
        ...
        // Truncation is a pure function of (name, maxNameW); cache it so the
        // per-character measuring loop doesn't run every frame for every setting row.
        TruncatedName cachedName = NAME_TRUNCATION_CACHE.get(name);
        if (cachedName == null || cachedName.maxWidth() != maxNameW) {
            float fullW = Fonts.REGULAR.getWidth(name, 6.5f);
            String shown = name;
            float shownW = fullW;
            if (fullW > maxNameW && maxNameW > 0) {
                ...
                shown = truncated + dots;
                shownW = Fonts.REGULAR.getWidth(shown, 6.5f);
            }
            if (NAME_TRUNCATION_CACHE.size() > 512) {
                NAME_TRUNCATION_CACHE.clear();
            }
            cachedName = new TruncatedName(maxNameW, shown, shownW);
            NAME_TRUNCATION_CACHE.put(name, cachedName);
        }
        String name2 = cachedName.text();
        float nameW = cachedName.width();
        name = name2;
    ...
    private void renderFloatSetting(DrawContext context, float panelX, float settingY, int alpha, int colorTheme, double mouseX, double mouseY, SliderSetting floatSetting, ClickGuiState state) {
