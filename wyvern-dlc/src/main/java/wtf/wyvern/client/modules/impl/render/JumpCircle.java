...
public final class JumpCircle extends Module implements IMinecraft {
    ...
    private final SliderSetting radius = new SliderSetting("Радиус", 1.5f, 0.5f, 4.0f, 0.1f);
    private final SliderSetting speed = new SliderSetting("Скорость", 1.0f, 1.0f, 5.0f, 0.1f);
    private final SliderSetting fadeSpeed = new SliderSetting("Скорость исчезновения", 1.5f, 1.0f, 5.0f, 0.1f);
    private final SliderSetting glowStrength = new SliderSetting("Сила свечения", 1.0f, 0.25f, 2.0f, 0.05f);
    ...
    private void renderGlowCircle(MatrixStack matrices, Vec3d cameraPos, CircleData circle, float progress, float alpha, long now) {
        ...
        int coreColor = baseTheme.mulAlpha(alphaBoost).getRGB();
        float glow = glowStrength.getCurrent();
        int innerGlow = baseTheme.mulAlpha(MathHelper.clamp(alphaBoost * 0.34f * glow, 0f, 1f)).getRGB();
        int middleGlow = baseTheme.mulAlpha(MathHelper.clamp(alphaBoost * 0.20f * glow, 0f, 1f)).getRGB();
        int outerGlow = baseTheme.mulAlpha(MathHelper.clamp(alphaBoost * 0.10f * glow, 0f, 1f)).getRGB();
        ...
        float innerHalf = finalScale * 0.56f;
        float middleHalf = finalScale * 0.64f;
        float outerHalf = finalScale * 0.74f;

        BufferBuilder buffer = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
        addTexturedQuad(buffer, matrix, -outerHalf, -outerHalf, outerHalf, outerHalf, outerGlow, outerGlow);
        addTexturedQuad(buffer, matrix, -middleHalf, -middleHalf, middleHalf, middleHalf, middleGlow, middleGlow);
        addTexturedQuad(buffer, matrix, -innerHalf, -innerHalf, innerHalf, innerHalf, innerGlow, innerGlow);
        addTexturedQuad(buffer, matrix, -half, -half, half, half, coreColor, coreColor);
