...
@ModuleAnnotation(
    ...
    description = "Кидает эндер-жемчуг в выбранную цель"
)
    ...
    private void onPacket(EventPacket e) {
        ...
        if (e.isReceive() && e.getPacket() instanceof EntitySpawnS2CPacket packet) {
            if (packet.getEntityType() == EntityType.ENDER_PEARL) {
                ...
                // Пакет приходит в network-потоке (ClientConnection#handlePacket, HEAD).
                // Поворот/interactItem обязаны выполняться в основном потоке, иначе перл не кидается.
                mc.execute(() -> throwAtPearl(pearlPos, velocity));
            }
        }
    }

    private void throwAtPearl(Vec3d pearlPos, Vec3d velocity) {
        if (mc.player == null || mc.world == null || !this.isEnabled()) return;
