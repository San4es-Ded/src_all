package wtf.wyvern.client.modules.impl.movement;

import net.minecraft.entity.effect.StatusEffects;
import net.minecraft.network.packet.c2s.play.PlayerMoveC2SPacket;
import net.minecraft.network.packet.s2c.play.PlayerPositionLookS2CPacket;
import net.minecraft.util.math.MathHelper;
import net.minecraft.util.math.Vec3d;
import wtf.wyvern.client.modules.api.Category;
import wtf.wyvern.client.modules.api.Module;
import wtf.wyvern.client.modules.api.ModuleAnnotation;
import wtf.wyvern.core.eventbus.EventTarget;
import wtf.wyvern.core.events.impl.other.EventTick;
import wtf.wyvern.core.events.impl.player.EventOnTravelPost;
import wtf.wyvern.core.events.impl.server.EventPacket;
import wtf.wyvern.utility.game.other.NetworkUtils;
import wtf.wyvern.utility.interfaces.IMinecraft;

@ModuleAnnotation(
        name = "GrimGlide",
        category = Category.MOVEMENT,
        description = "Обход Grim-проверок при полёте на элитрах"
)
public final class GrimGlide extends Module implements IMinecraft {
    public static final GrimGlide INSTANCE = new GrimGlide();

    private int flagTicks;
    private boolean isFlag;

    private GrimGlide() {
    }

    @EventTarget
    private void onPacket(EventPacket event) {
        if (mc.player == null) return;

        if (event.getPacket() instanceof PlayerPositionLookS2CPacket) {
            flagTicks = 2;
            isFlag = true;
        }

        if (event.getPacket() instanceof PlayerMoveC2SPacket) {
            if (mc.player.isGliding()) {
                if (flagTicks == 0 && !isFlag) {
                    NetworkUtils.sendSilentPacket(new PlayerMoveC2SPacket.OnGroundOnly(true, true));
                    event.cancel();
                }
            }
            isFlag = false;
        }
    }

    @EventTarget
    private void onTick(EventTick event) {
        if (flagTicks > 0) flagTicks--;
    }

    @EventTarget
    private void onTravelPost(EventOnTravelPost event) {
        if (mc.player == null) return;

        Vec3d oldVelocity = mc.player.getVelocity();
        Vec3d look = mc.player.getRotationVector();
        float f = mc.player.getPitch() * 0.017453292F;
        double d = Math.sqrt(look.x * look.x + look.z * look.z);
        double e = oldVelocity.horizontalLength();
        boolean bl = mc.player.getVelocity().y <= 0.0;
        double g = bl && mc.player.hasStatusEffect(StatusEffects.SLOW_FALLING) ? Math.min(mc.player.getFinalGravity(), 0.01) : mc.player.getFinalGravity();
        double h = MathHelper.square(Math.cos((double) f));
        oldVelocity = oldVelocity.add(0.0, g * (-1.0 + h * 0.75), 0.0);
        double i;
        if (oldVelocity.y < 0.0 && d > 0.0) {
            i = oldVelocity.y * -0.1 * h;
            oldVelocity = oldVelocity.add(look.x * i / d, i, look.z * i / d);
        }

        if (f < 0.0F && d > 0.0) {
            i = e * (double) (-MathHelper.sin(f)) * 0.04F;
            oldVelocity = oldVelocity.add(-look.x * i / d, i * 3.2, -look.z * i / d);
        }

        if (d > 0.0) {
            oldVelocity = oldVelocity.add((look.x / d * e - oldVelocity.x) * 0.1, 0.0, (look.z / d * e - oldVelocity.z) * 0.1);
        }

        double yaw = Math.toRadians(mc.player.getYaw());
        double xt = -Math.sin(yaw);
        double zt = Math.cos(yaw);

        if (flagTicks >= 1) {
            double bst = 0.09F;
            event.setOldVelocity(oldVelocity.multiply(0.99F, 0.9800000190734863, 0.99F).add(xt * bst, 0.03F, zt * bst));
        } else {
            event.setOldVelocity(oldVelocity.multiply(0.3F, 0.3F, 0.3F));
        }
    }
}