...
@ModuleAnnotation(
        name = "FreeCam",
        description = "Обзор местности за свободную камеру",
...
public final class FreeCam extends Module {
   ...
   public void onWorldRender(EventRender3D e) {
      if (this.pos != null) {
         Render3DUtil.drawBox(new Box(this.pos.x - (double)(mc.player.getWidth() / 2.0F), this.pos.y, this.pos.z - (double)(mc.player.getWidth() / 2.0F), this.pos.x + (double)(mc.player.getWidth() / 2.0F), this.pos.y + (double)mc.player.getHeight(), this.pos.z + (double)(mc.player.getWidth() / 2.0F)), WYVERN.getThemeManager().getClientColor(90).getRGB(), 1.0F);
      }
   }
