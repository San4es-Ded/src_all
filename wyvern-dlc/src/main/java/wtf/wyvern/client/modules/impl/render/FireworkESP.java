...
public final class FireworkESP extends Module implements IMinecraft {
    ...
    private final SliderSetting interval = new SliderSetting("Интервал", 100.0f, 10.0f, 1000.0f, 10.0f);
    private final SliderSetting lifetime = new SliderSetting("Время жизни", 1000.0f, 100.0f, 5000.0f, 100.0f);
    ...
    private void onRender3D(EventRender3D event) {
        ...
        for (FireworkRocketEntity entity : RenderFrameCache.fireworks()) {
            if (entity.isAlive()) {
    ...
    private void onRender2D(EventRender2D event) {
        ...
        for (Map.Entry<Integer, FireworkData> entry : fireworks.entrySet()) {
            ...
            for (TrailPoint p : data.points) {
                ...
                String text = formatSeconds(p.ageSec);

                renderIconRect(event, matrices, FIREWORK_ICON, screen, progress, text);
            }
            ...
            if (entity instanceof FireworkRocketEntity && entity.isAlive()) {
                ...
                if (screen.z >= 0.0 && screen.z <= 1.0) {
                    String text = formatSeconds(entity.age / 20.0f);
                    renderIconRect(event, matrices, FIREWORK_ICON, screen, 1.0f, text);
                }
            }
        }
    }
    ...
    private static String formatSeconds(float seconds) {
        int tenths = Math.max(0, Math.round(seconds * 10.0f));
