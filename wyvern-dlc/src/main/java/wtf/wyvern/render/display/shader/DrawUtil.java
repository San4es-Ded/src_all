...
import wtf.wyvern.render.display.base.CustomSprite;
...
public final class DrawUtil implements IWindow {
   ...
   private static GlProgram mainMenuProgram;
   ...
   /**
    * The HUD blur must always sample an immutable image of the world.  Sampling a framebuffer
    * after drawing a previous blurred widget turns the effect into a feedback loop, which is
    * especially visible when Iris owns the main render target.
    */
   private static boolean blurSnapshotValid;

   public static void initializeShaders() {
      rectangleProgram = new GlProgram(Wyvern.id("rectangle/data"), VertexFormats.POSITION_COLOR);
      squircleProgram = new GlProgram(Wyvern.id("squircle/data"), VertexFormats.POSITION_COLOR);
      squircleTextureProgram = new GlProgram(Wyvern.id("squircle_texture/data"), VertexFormats.POSITION_TEXTURE_COLOR);
      roundedTextureProgram = new GlProgram(Wyvern.id("texture/data"), VertexFormats.POSITION_TEXTURE_COLOR);
      borderProgram = new GlProgram(Wyvern.id("border/data"), VertexFormats.POSITION_COLOR);
      figmaBorderProgram = new GlProgram(Wyvern.id("corner/data"), VertexFormats.POSITION_COLOR);
      loadingProgram = new GlProgram(Wyvern.id("loading/data"), VertexFormats.POSITION_COLOR);
      gradientRectangleProgram = new GlProgram(Wyvern.id("gradient_rectangle/data"), VertexFormats.POSITION_COLOR);
      blurProgram = new GlProgram(Wyvern.id("blur/data"), VertexFormats.POSITION_COLOR);
      metaballsProgram = new GlProgram(Wyvern.id("metanoise/data"), VertexFormats.POSITION_COLOR);
      mainMenuProgram = new GlProgram(Wyvern.id("main_menu/data"), VertexFormats.POSITION_COLOR);
   }
   ...
   /** Call once at the beginning of every HUD/screen frame. */
   public static void beginBlurFrame() {
      blurSnapshotValid = false;
   }

   private static boolean captureBlurSnapshot(Framebuffer source, SimpleFramebuffer snapshot) {
      if (source.textureWidth <= 0 || source.textureHeight <= 0) {
         return false;
      }

      try {
         // Do a texture copy instead of rendering the main framebuffer into another framebuffer.
         // Iris can replace the main target while a shader pack is active; rendering it here makes
         // the source include our own previous HUD passes. glCopyTexSubImage2D copies only the
         // completed scene and therefore keeps the blur stable with both Iris and vanilla.
         source.beginRead();
         GL11.glBindTexture(GL11.GL_TEXTURE_2D, snapshot.getColorAttachment());
         GL11.glCopyTexSubImage2D(GL11.GL_TEXTURE_2D, 0, 0, 0, 0, 0,
                 source.textureWidth, source.textureHeight);
         GL11.glBindTexture(GL11.GL_TEXTURE_2D, 0);
         source.endRead();
         source.beginWrite(false);
         return true;
   ...
   public static void drawMainMenuBackground(MatrixStack matrices, float width, float height, float time) {
      if (mainMenuProgram == null || mainMenuProgram.backingProgram == null) {
         drawRoundedRect(matrices, 0.0F, 0.0F, width, height, BorderRadius.ZERO,
                 new ColorRGBA(5, 6, 7), new ColorRGBA(18, 19, 20),
                 new ColorRGBA(18, 19, 20), new ColorRGBA(5, 6, 7));
         return;
      }
      ...
      mainMenuProgram.use();
      mainMenuProgram.findUniform("Size").set(width, height);
      ColorRGBA primary = Wyvern.getInstance().getThemeManager().getCurrentTheme().getColor();
      ColorRGBA secondary = Wyvern.getInstance().getThemeManager().getCurrentTheme().getSecondColor();
      if (mainMenuProgram.findUniform("ThemeColor") != null) {
         mainMenuProgram.findUniform("ThemeColor").set(
                 primary.getRed() / 255.0F, primary.getGreen() / 255.0F,
                 primary.getBlue() / 255.0F, primary.getAlpha() / 255.0F);
      }
      if (mainMenuProgram.findUniform("ThemeColor2") != null) {
         mainMenuProgram.findUniform("ThemeColor2").set(
                 secondary.getRed() / 255.0F, secondary.getGreen() / 255.0F,
                 secondary.getBlue() / 255.0F, secondary.getAlpha() / 255.0F);
      }
      // GLSL is allowed to optimize uniforms that are not used by a custom
      // menu shader. Keep this screen resilient to such shader variants.
      if (mainMenuProgram.findUniform("Time") != null) {
         mainMenuProgram.findUniform("Time").set(time);
      }
      drawSetup();
      BufferBuilder builder = RenderSystem.renderThreadTesselator().begin(DrawMode.QUADS, VertexFormats.POSITION_COLOR);
      builder.vertex(matrix, 0.0F, 0.0F, 0.0F).color(-1);
      builder.vertex(matrix, 0.0F, height, 0.0F).color(-1);
      builder.vertex(matrix, width, height, 0.0F).color(-1);
      builder.vertex(matrix, width, 0.0F, 0.0F).color(-1);
      BufferRenderer.drawWithGlobalProgram(builder.end());
      drawEnd();
      matrices.pop();
   }
   ...
   public static void drawGlow(MatrixStack matrixStack, float x, float y, float width, float height, int glowRadius) {
      Render2DUtil.drawGradientBlurredShadow(matrixStack, x, y, width, height, glowRadius, Gradient.of(Wyvern.getInstance().getThemeManager().getCurrentTheme().getColor(), Wyvern.getInstance().getThemeManager().getCurrentTheme().getColor(), Wyvern.getInstance().getThemeManager().getCurrentTheme().getSecondColor(), Wyvern.getInstance().getThemeManager().getCurrentTheme().getSecondColor()));
   }
   ...
   public static void drawBlur(MatrixStack matrices, float x, float y, float width, float height, float blurRadius, BorderRadius borderRadius, ColorRGBA color) {
      // Animated widgets frequently keep rendering while fully transparent or
      // collapsed. Avoid a framebuffer copy and an expensive shader pass for them.
      if (width <= 0.0F || height <= 0.0F || color == null || color.getAlpha() <= 0) {
         return;
      }
      ...
      if (!blurSnapshotValid) {
         blurSnapshotValid = captureBlurSnapshot(MAIN_FBO, fbo);
      }
      if (!blurSnapshotValid || blurProgram == null || !blurProgram.isLoaded()) {
         return;
      }
      ...
      blurProgram.findUniform("InvResolution").set(1.0F / (float)fbo.textureWidth, 1.0F / (float)fbo.textureHeight);
