package wtf.wyvern.render.display;
...
public final class Render2DUtil implements IMinecraft {
   ...
   private static final ExecutorService GLOW_GENERATOR = Executors.newSingleThreadExecutor(r -> {
      Thread thread = new Thread(r, "wyvern-glow-generator");
      thread.setDaemon(true);
      return thread;
   });
   private static final java.util.Set<GlowKey> PENDING_GLOWS = java.util.concurrent.ConcurrentHashMap.newKeySet();
   ...
   public static void drawGradientBlurredShadow(MatrixStack matrices, float x, float y, float width, float height, int blurRadius, Gradient gradient) {
      ...
      x -= (float)blurRadius;
      y -= (float)blurRadius;
      GlowKey exactKey = new GlowKey((int)width, (int)height, blurRadius);
      GlowKey existing;
      GlowRect glowRect = (GlowRect)glowCache.get(exactKey);
      if (glowRect != null) {
         // Exact texture already cached: skip the O(n) similarity scan entirely.
         existing = exactKey;
      } else {
         existing = findSimilarKey((int)width, (int)height, blurRadius);
         if ((existing == null || (int)(Math.abs((float)existing.width() - width) + Math.abs((float)existing.height() - height) + (float)Math.abs(existing.blurRadius() - blurRadius)) >= 5)
               && PENDING_GLOWS.add(exactKey)) {
            // Generate on the background executor: the CPU gaussian blur and the PNG
            // encode/decode used to run on the render thread and stalled the frame.
            // The texture appears a few frames later exactly as before, so visuals
            // are unchanged - only the stall is gone.
            GLOW_GENERATOR.execute(() -> {
               try {
                  BufferedImage original = new BufferedImage(exactKey.width(), exactKey.height(), 2);
                  Graphics g = original.getGraphics();
                  g.setColor(new Color(-1));
                  g.fillRect(blurRadius, blurRadius, exactKey.width() - blurRadius * 2, exactKey.height() - blurRadius * 2);
                  g.dispose();
                  GaussianFilter op = new GaussianFilter((float)blurRadius);
                  BufferedImage blurred = op.filter(original, (BufferedImage)null);
                  GlowRect generated = new GlowRect(blurred);
                  mc.execute(() -> {
                     glowCache.put(exactKey, generated);
                     PENDING_GLOWS.remove(exactKey);
                  });
               } catch (Throwable t) {
                  PENDING_GLOWS.remove(exactKey);
               }
         ...
         glowRect = existing != null ? (GlowRect)glowCache.get(existing) : null;
      }
   ...
   public static void registerTexture(Texture i, byte[] content) {
      try {
         ...
         data.flip();
         // Decode may run on a background thread, but the GL texture must be created
         // on the render thread: an off-thread NativeImageBackedTexture defers its
         // upload to end-of-frame, which let an un-uploaded (black) texture be drawn
         // for one frame after the async glow generation landed in the cache.
         NativeImage image = NativeImage.read(data);
         mc.execute(() -> {
            mc.getTextureManager().registerTexture(i.getId(), new NativeImageBackedTexture(image));
         });
      } catch (Exception var4) {
      }
