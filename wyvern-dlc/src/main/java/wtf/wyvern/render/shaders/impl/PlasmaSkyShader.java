package wtf.wyvern.render.shaders.impl;

import wtf.wyvern.render.shaders.Shader;

public class PlasmaSkyShader extends Shader {

    private static PlasmaSkyShader INSTANCE;

    public static PlasmaSkyShader getInstance() {
        if (INSTANCE == null) INSTANCE = new PlasmaSkyShader();
        return INSTANCE;
    }
    ...
    public String getCode() {
        return """
                ...
                void main() {
                    vec2 ndc = rawPos.xy;
                    vec3 direction = normalize(look - left * ndc.x * aspect * fovTan + up * ndc.y * fovTan);
                    ...
                    if (mode == 1) {
                        ...
                        color = renderLunarisBloom(direction, t);
                    } else if (mode == 5) {
                        color = renderVoidCrown(direction, t);
                    } else {
