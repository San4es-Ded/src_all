...
public final class MessageUtil implements IMinecraft {
   public static void displayMessage(LogLevel level, Object message) {
      Text icon = createGradientText("Wyvern", Wyvern.getInstance().getThemeManager().getCurrentTheme().getColor(), Wyvern.getInstance().getThemeManager().getCurrentTheme().getSecondColor());
   ...
   public static enum LogLevel {
      ...
      private static LogLevel[] $values() {
         return new LogLevel[]{WARN, ERROR, INFO};
      }
   }
}
