...
@FastNative
public class AuraRotations implements IMinecraft {
    ...
    public void updateHVH(Rotation angle, boolean elytraVisual) {
    ...
    public void updateLegendsGrief(LivingEntity target, Rotation angle, float distance, float predict, boolean elytraVisual) {
        if (mc.player.isGliding()) {
            ...
            float microYawNoise = (rng.nextFloat() - 0.5F) * 0.022F;
            float microPitchNoise = (rng.nextFloat() - 0.5F) * 0.013F;
            ...
            float microYawNoise = (rng.nextFloat() - 0.5F) * 0.018F;
            float microPitchNoise = (rng.nextFloat() - 0.5F) * 0.009F;
