package wtf.wyvern.utility.predict;
...
public final class PredictUtils {
    ...
    public static Vec3d getPredicted(LivingEntity entity, float ticks) {
        return predict(entity, entity.getPos(), ticks);
    }

    public static Vec3d getMovement(LivingEntity entity) {
        if (entity == null) {
            return Vec3d.ZERO;
        }
        Vec3d tickMotion = new Vec3d(
                entity.getX() - entity.prevX,
                entity.getY() - entity.prevY,
                entity.getZ() - entity.prevZ
        );
        Vec3d velocity = entity.getVelocity();
        if (tickMotion.lengthSquared() < 1.0E-6D) {
            return velocity;
        }
        if (velocity.lengthSquared() > 1.0E-6D) {
            return tickMotion.multiply(0.72D).add(velocity.multiply(0.28D));
        }
        return tickMotion;
    }
}
