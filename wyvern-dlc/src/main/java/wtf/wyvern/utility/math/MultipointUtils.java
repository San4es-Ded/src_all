...
public class MultipointUtils implements IClient {
   ...
   /**
    * getNearestPoint runs a ~900-raycast grid scan and is invoked several times per
    * client frame with identical inputs (rotation update loop + attack check +
    * validity check for the same target). The result is memoized for the current
    * client frame, keyed by every input that affects the outcome (target, eye
    * position, hitbox, distance). World block updates are applied between frames,
    * so a new frame always recomputes; the cache is cleared on world change /
    * disconnect from onClientFrame so no entity or world is retained.
    */
   private static Entity nearestPointTarget;
   private static Vec3d nearestPointEye;
   private static Box nearestPointBox;
   private static double nearestPointDistance;
   private static long nearestPointFrame = Long.MIN_VALUE;
   private static Vec3d nearestPointResult;
   private static long frameCounter;
   private static Object cachedWorld;

   /** Called once per MinecraftClient.render pass (see MinecraftClientMixin). */
   public static void onClientFrame() {
      frameCounter++;
      if (mc == null) {
         return;
      }
      if (mc.world == null || mc.world != cachedWorld) {
         cachedWorld = mc.world;
         nearestPointTarget = null;
         nearestPointEye = null;
         nearestPointBox = null;
         nearestPointResult = null;
         nearestPointFrame = Long.MIN_VALUE;
      }
   }
   ...
   public static Vec3d getNearestPoint(Entity target, double distance) {
      ...
      if (nearestPointResult != null && nearestPointTarget == target && nearestPointFrame == frame
            && nearestPointDistance == distance && eyePos.equals(nearestPointEye) && box.equals(nearestPointBox)) {
      ...
      Vec3d result = bestPoint == null ? target.getBoundingBox().getCenter() : bestPoint;
      nearestPointTarget = target;
      nearestPointEye = eyePos;
      nearestPointBox = box;
      nearestPointDistance = distance;
      nearestPointFrame = frame;
