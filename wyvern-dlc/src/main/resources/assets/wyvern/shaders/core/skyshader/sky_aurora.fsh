...
void main() {
    ...
    vec3 rd = normalize(vec3(screen * tanFov, 1.0));
    rd = normalize(rotY(u_CameraDir.x) * rotX(u_CameraDir.y) * rd);
    ...
    float t = u_Time * 0.45;
    float skyScale = max(0.35, u_Scale);
    vec2 uv = rd.xz * (4.8 * skyScale);
    uv.y += rd.y * (2.2 * skyScale);
