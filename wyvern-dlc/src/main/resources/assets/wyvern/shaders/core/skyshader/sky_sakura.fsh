...
void main() {
    ...
    vec3 rayView = normalize(vec3(screen.x * tanHalfFov * aspect, screen.y * tanHalfFov, 1.0));
    vec3 rd = normalize(rotY(u_CameraDir.x) * rotX(u_CameraDir.y) * rayView);
    float t = u_Time;
    ...
    float neb1 = fbm(nebulaP);
    float neb2 = fbm(nebulaP * 1.8 + vec3(9.3, 2.1, -4.7));
    float dust = fbm(nebulaP * 2.6 + vec3(-2.7, 7.9, 5.4));
