...
public final class HeadlessBotCommand extends Command {
   public HeadlessBotCommand() {
      super("bot", "Управление хедлесс-ботами", ".bot connect <ник> <ip[:port]> | remove | chat | control <ник> | return | list | proxy <ник> <прокси|clear> | clear");
      ...
      this.handle("proxy", () -> HeadlessBotEngine.process().stream().map(HeadlessBotEngine.ServerEntry::name).collect(Collectors.toList()));
      ...
      if (var1.length != 0 && !var1[0].equalsIgnoreCase("help")) {
         switch (var1[0].toLowerCase()) {
            case "connect":
               ...
               HeadlessBotConnector.handle(var1[1], var1[2], var1.length < 4 ? "" : var1[3]);
               break;
            case "proxy":
               if (var1.length < 2) {
                  ChatLogger.handle("§c[Bot] Использование: .bot proxy <ник> [host:port:user:pass | clear]");
                  return;
               }

               HeadlessBotEngine.ServerEntry var8 = HeadlessBotEngine.handle(var1[1]);
               if (var8 == null) {
               ...
               if (var1.length < 3) {
                  String var9 = var8.proxy();
                  ChatLogger.handle(
                     "§7[Bot] Прокси §f" + var8.name() + "§7: " + (var9 == null || var9.isBlank() ? "§8не задан" : "§f" + var9)
                  );
                  return;
               }

               if (var1[2].equalsIgnoreCase("clear") || var1[2].equalsIgnoreCase("none") || var1[2].equals("-")) {
                  HeadlessBotEngine.proxy(var1[1], "");
                  ChatLogger.handle("§7[Bot] Прокси §f" + var8.name() + "§7 сброшен §8(применится при следующем подключении)");
                  return;
               }

               HeadlessBotEngine.proxy(var1[1], var1[2]);
               ChatLogger.handle(
                  "§7[Bot] Прокси §f" + var8.name() + "§7: §f" + HeadlessBotEngine.proxy(var1[1]) + " §8(применится при следующем подключении)"
               );
               break;
            case "remove":
               if (var1.length < 2) {
         ...
         ChatLogger.handle("  §f.bot connect §7<ник> <ip[:port]/домен> [прокси] §8— подключить бота");
         ...
         ChatLogger.handle("  §f.bot proxy §7<ник> [host:port:user:pass|clear] §8— прокси бота");
         ChatLogger.handle("  §f.bot remove §7<ник> §8— отключить бота");
         ChatLogger.handle("  §f.bot clear §8— отключить всех");
         ChatLogger.handle("§8Прокси берутся по одному на бота из §fproxies.txt §8в папке клиента.");
      }
   }
}
