...
public class HandShaderProgram {
    private static final HandShaderProgram INSTANCE = new HandShaderProgram();
    ...
    public void render(float f, float f2, float f3, Color color, int n, float f4) {
        this.init();
        if (!this.initFailed && this.programId != -1) {
            ...
            int n6 = GL11.glGetInteger((int)3041);
            int n7 = GL11.glGetInteger((int)3040);
            ...
            float f5 = (float)System.nanoTime() / 1.0E9f - this.startTime;
            GL20.glUniform1f((int)this.uTime, (float)f5);
            ...
            GL20.glUniform1f((int)this.uOpacity, (float)f4);
            ...
            GL11.glBlendFunc((int)n6, (int)n7);
            if (!bl) {
    ...
    private int compileShader(int n, String string) {
        ...
        if (GL20.glGetShaderi((int)n2, (int)35713) == 0) {
            System.err.println(HandShaderProgram.$sf$2(string, GL20.glGetShaderInfoLog((int)n2)));
        }
        return n2;
    }
    ...
    private String readShaderSource(String string) {
        try {
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(HandShaderProgram.class.getResourceAsStream(HandShaderProgram.$sf$3(string)))));
            String string2 = bufferedReader.lines().collect(Collectors.joining("\n"));
            bufferedReader.close();
            return string2;
        }
        catch (IOException iOException) {
            throw new RuntimeException(HandShaderProgram.$sf$4(string), iOException);
        }
    }

    public static HandShaderProgram get() {
        return INSTANCE;
    }
    ...
    private void init() {
        if (this.programId == -1 && !this.initFailed) {
            try {
                int n = this.compileShader(35633, "hand_shader.vert");
                int n2 = this.compileShader(35632, "hand_shader.frag");
                ...
                if (GL20.glGetProgrami((int)this.programId, (int)35714) == 0) {
                    System.err.println(HandShaderProgram.$sf$0(GL20.glGetProgramInfoLog((int)this.programId)));
                ...
                this.uOpacity = GL20.glGetUniformLocation((int)this.programId, (CharSequence)"Opacity");
                ...
                System.err.println(HandShaderProgram.$sf$1(String.valueOf(exception)));
                this.initFailed = true;
            }
        }
    }
    ...
    private static /* synthetic */ String $sf$0(String string) {
        return "[Haron] HandShader link failed: " + string;
    }
    ...
    private static /* synthetic */ String $sf$1(String string) {
        return "[Haron] HandShader init failed: " + string;
    }

    private static /* synthetic */ String $sf$2(String string, String string2) {
        return "[Haron] HandShader compile failed (" + string + "): " + string2;
    }
}
