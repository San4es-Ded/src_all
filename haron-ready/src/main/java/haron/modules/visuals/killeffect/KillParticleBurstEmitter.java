...
public final class KillParticleBurstEmitter {
    ...
    public void spawnBurst(Vec3d vec3d, int n, int n2, float f, int n3, float f2) {
        ...
        for (int i = 0; i < n4; ++i) {
            minecraftClient.world.addParticle((ParticleEffect)ParticleTypes.END_ROD, vec3d.x, vec3d.y, vec3d.z, threadLocalRandom.nextDouble(-f3, f3), threadLocalRandom.nextDouble((double)(-f3) * 0.25, (double)f3 * 0.55) - (double)f2, threadLocalRandom.nextDouble(-f3, f3));
        }
    }
