...
// stack = Unit.INSTANCE;
    ...
    private final void init() {
        ...
        try {
            ...
            if ((current = uniformBuffer) == null || current.isClosed() || current.size() < 128L) {
                this.closeUniform();
                uniformBuffer = RenderSystem.getDevice().createBuffer(AmbienceFogRenderer::init$lambda$0, 136, 128L);
            }
        }
        catch (Throwable throwable) {
            disabledAfterError = true;
    ...
    private static final String apply$lambda$1() {
        return "kimiko:ambience_fog";
    }

    private static final String init$lambda$0() {
        return "kimiko:ambience_fog_uniforms";
    }
