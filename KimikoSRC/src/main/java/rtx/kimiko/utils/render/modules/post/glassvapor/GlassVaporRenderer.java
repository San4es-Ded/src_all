 ...
 */
package rtx.kimiko.utils.render.modules.post.glassvapor;
    ...
    public static final void apply(@Nullable Framebuffer renderTarget, @NotNull float[] uniform) {
        ...
        try {
            ...
            try {
                ...
                boolean bl = false;
                ByteBuffer data = stack.calloc(15424);
                for (int i = 0; i < 3856; ++i) {
                    data.putFloat(i * 4, i < uniform.length ? uniform[i] : 0.0f);
                }
                data.position(0);
                encoder.writeToBuffer(currentUniform.slice(0L, 15424L), data);
// stack = Unit.INSTANCE;
            }
            catch (Throwable bl) {
                throwable = bl;
            ...
            Supplier<String> supplier = GlassVaporRenderer::apply$lambda$1;
            ...
            try {
                ...
                renderPass.setUniform("Vapor", currentUniform);
    ...
    private final void init() {
        ...
        try {
            GpuBuffer current;
            if (pipeline == null) {
                pipeline = RenderPipelines.register((RenderPipeline)RenderPipeline.builder((RenderPipeline.Snippet[])new RenderPipeline.Snippet[0]).withLocation(PIPELINE_ID).withVertexShader(VERTEX).withFragmentShader(SHADER).withVertexFormat(VertexFormats.EMPTY, VertexFormat.DrawMode.TRIANGLES).withUniform("Vapor", UniformType.UNIFORM_BUFFER).withUniform("ThemeWaveParams", UniformType.UNIFORM_BUFFER).withSampler("Scene").withSampler("DepthSampler").withBlend(BlendFunction.TRANSLUCENT).withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST).withDepthWrite(false).withCull(false).build());
            }
            if ((current = uniformBuffer) == null || current.isClosed() || current.size() < 15424L) {
                this.closeUniform();
                uniformBuffer = RenderSystem.getDevice().createBuffer(GlassVaporRenderer::init$lambda$0, 136, 15424L);
            }
        }
        catch (Throwable throwable) {
            disabledAfterError = true;
    ...
    private final boolean ensureSceneCopy(int width, int height) {
        ...
        this.closeTargets();
        GpuTexture gpuTexture = sceneCopyTexture = device.createTexture(GlassVaporRenderer::ensureSceneCopy$lambda$0, 5, TextureFormat.RGBA8, width, height, 1, 1);
        Intrinsics.checkNotNull((Object)gpuTexture);
        sceneCopyTextureView = device.createTextureView(gpuTexture);
        GpuTexture gpuTexture2 = depthCopyTexture = device.createTexture(GlassVaporRenderer::ensureSceneCopy$lambda$1, 5, TextureFormat.DEPTH32, width, height, 1, 1);
        Intrinsics.checkNotNull((Object)gpuTexture2);
