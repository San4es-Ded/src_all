...
implements SimpleGuiElementRenderState {
    ...
    public GlassRenderState(@NotNull Matrix3x2f pose, @NotNull BuiltGlass glass, @Nullable ScreenRect scissorArea, @NotNull BlurCapture capture) {
        Intrinsics.checkNotNullParameter((Object)pose, (String)"pose");
        Intrinsics.checkNotNullParameter((Object)glass, (String)"glass");
        Intrinsics.checkNotNullParameter((Object)capture, (String)"capture");
        this.glass = glass;
        this.scissorArea = scissorArea;
        this.capture = capture;
        this.pose = PoseCache.snapshot(pose);
        ScreenRect screenRect2 = new ScreenRect(Math.round(this.glass.x()), Math.round(this.glass.y()), Math.round(this.glass.width()), Math.round(this.glass.height())).transformEachVertex((Matrix3x2fc)this.pose);
    ...
    public void setupVertices(@NotNull VertexConsumer consumer) {
        ...
        int batchIndex = GlassBatch.INSTANCE.reserve(new GlassEntry(this.glass, this.capture));
        if (batchIndex < 0) {
            return;
        }
        float x0 = this.glass.x();
        float y0 = this.glass.y();
        float x1 = this.glass.x() + this.glass.width();
        float y1 = this.glass.y() + this.glass.height();
    ...
    private final void vertex(VertexConsumer consumer, float x, float y, int batchIndex) {
        consumer.vertex((Matrix3x2fc)this.pose, x, y).color(this.glass.color()).lineWidth((float)(batchIndex + 1));
    }

    @NotNull
    public RenderPipeline pipeline() {
        return GlassBatch.PIPELINE;
    }
