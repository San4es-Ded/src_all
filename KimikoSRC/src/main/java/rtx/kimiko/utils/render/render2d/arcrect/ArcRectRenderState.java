...
implements SimpleGuiElementRenderState {
    ...
    public ArcRectRenderState(@NotNull Matrix3x2f pose, @NotNull BuiltArcRect arc, @Nullable ScreenRect scissorArea) {
        Intrinsics.checkNotNullParameter((Object)pose, (String)"pose");
        Intrinsics.checkNotNullParameter((Object)arc, (String)"arc");
        this.arc = arc;
        this.scissorArea = scissorArea;
        this.pose = PoseCache.snapshot(pose);
        float extent = this.arc.extent();
        int boundsX = (int)Math.floor(this.arc.centerX() - extent);
        int boundsY = (int)Math.floor(this.arc.centerY() - extent);
    ...
    public void setupVertices(@NotNull VertexConsumer consumer) {
        ...
        int batchIndex = ArcRectBatch.INSTANCE.reserve(this.arc);
        if (batchIndex < 0) {
            return;
        }
        float extent = this.arc.extent();
        float x0 = this.arc.centerX() - extent;
        float y0 = this.arc.centerY() - extent;
        float x1 = this.arc.centerX() + extent;
        float y1 = this.arc.centerY() + extent;
