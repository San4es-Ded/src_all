 ...
 */
package rtx.kimiko.utils.render.modules.post.jumpdistort;
...
public final class JumpDistortRenderer {
    ...
    public static final void apply(@Nullable Framebuffer renderTarget, @NotNull float[] uniform) {
        ...
        RenderProfiler.Scope scope = RenderProfiler.begin("world.jumpdistort");
        try {
            ...
            try {
                ...
                boolean bl = false;
                ByteBuffer data = stack.calloc(1904);
                for (int i = 0; i < 476; ++i) {
                    data.putFloat(i * 4, i < uniform.length ? uniform[i] : 0.0f);
                }
                data.position(0);
                encoder.writeToBuffer(currentUniform.slice(0L, 1904L), data);
// stack = Unit.INSTANCE;
            }
            catch (Throwable bl) {
                throwable = bl;
            ...
            Supplier<String> supplier = JumpDistortRenderer::apply$lambda$1;
    ...
    private final boolean ensureSceneCopy(int width, int height) {
        ...
        this.closeTargets();
        GpuTexture gpuTexture = sceneCopyTexture = device.createTexture(JumpDistortRenderer::ensureSceneCopy$lambda$0, 5, TextureFormat.RGBA8, width, height, 1, 1);
        Intrinsics.checkNotNull((Object)gpuTexture);
        sceneCopyTextureView = device.createTextureView(gpuTexture);
        GpuTexture gpuTexture2 = depthCopyTexture = device.createTexture(JumpDistortRenderer::ensureSceneCopy$lambda$1, 5, TextureFormat.DEPTH32, width, height, 1, 1);
        Intrinsics.checkNotNull((Object)gpuTexture2);
